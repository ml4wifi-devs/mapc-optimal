...
def plot_thr(names: List, throughput: List, xs: Array, scenario: str) -> None:
    ...
    for i, (name, thr) in enumerate(zip(names, throughput)):
        ...
        plt.plot(xs, mean, marker='o', markersize=2, label=name, c=colors[i])
...
if __name__ == '__main__':
    ...
    for scenario in results:
        ...
        for agent in scenario['agents']:
            ...
            runs = [np.array(run).reshape((-1, args.steps)).mean(axis=-1) for run in agent['runs']]
            throughput.append(np.array(runs))

        xs = np.arange(throughput[0].shape[-1]) * args.steps
